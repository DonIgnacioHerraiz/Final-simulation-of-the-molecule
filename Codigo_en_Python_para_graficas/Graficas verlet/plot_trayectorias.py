...
data_file = os.path.join(base_dir, "Resultados_simulacion", "DOBLE_POZO", "VERLET", filename)
param_file = os.path.join(base_dir, "PARAMETROS", "DOBLE_POZO", "VERLET", filename)
output_dir = os.path.join(base_dir, "Graficas", "DOBLE_POZO", "VERLET", "TRAYECTORIAS")
os.makedirs(output_dir, exist_ok=True)
output_file = os.path.join(output_dir, f"{filename_no_ext}.png")  # guardará V_1.png

# ──────────────────────────────
# Leer parámetros del archivo (robusto contra líneas con texto)
# ──────────────────────────────
params = {}
    ...
    for line in f:
        ...
        if len(parts) >= 2:
            try:
                ...
                # Ignorar líneas que no sean "clave numero"
                continue
